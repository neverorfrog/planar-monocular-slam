...
def initial_guess_error(
    ...
    squared_errors = np.sum(
        differences**2, axis=1
    )  # Sum of squared differences per landmark
    ...
    print(
        f"MAE per coordinate: X={mae_per_coordinate[0]:.4f}, Y={mae_per_coordinate[1]:.4f}, Z={mae_per_coordinate[2]:.4f}"
    )
    ...
    for i, axis in enumerate(["X", "Y", "Z"]):
        ...
        print(
            f"  {axis}-axis: Mean={np.mean(component_errors):.4f}, Std={np.std(component_errors):.4f}, "
